...
if(!categoria?.horario) return {aberto:true,mensagem:""};
...
const adicionarAoCarrinho = (produto) => {
  setCarrinho(prev => [
    ...
    {
      ...produto,
      // Guarda o texto "500g" ou "6 un." em um campo seguro para não quebrar a matemática
      medida: produto.quantidade && typeof produto.quantidade === "string" ? produto.quantidade : "",
      // Força o campo quantidade a ser estritamente o número 1 para o cálculo do carrinho funcionar
      quantidade: 1, 
      // Garante que o preço unitário seja repassado corretamente
...
const abrirConfigurador = (produto) => {
  ...
  // Se o item não tem opcionais (como o Balde de Longneck), passa ele direto para a correção acima
...
// CORRIGIDO: Agora envia o número real extraído do QR Code físico (numeroMesa) para o Firebase
...
{
mesa: numeroMesa, // <--- Aqui garante que vai o número real da mesa (ex: "5") e não "Principal"
itens:carrinho,
...
// Variáveis calculadas mantendo os estados reativos das abas
...
<button
onClick={()=>navigate(`/painel/cozinha/${empresa.slug}`)}
className="w-full bg-stone-900 text-amber-500 py-3 rounded-xl mb-3 font-bold">
🍳 Cozinha
</button>
<button
onClick={()=>navigate(`/painel/garcom/${empresa.slug}`)}
className="w-full bg-amber-800 text-white py-3 rounded-xl mb-3 font-bold">
🔔 Garçom
</button>
<button
onClick={()=>setMostrarMenuSecreto(false)}
className="w-full bg-stone-200 py-2 rounded-xl font-medium">
Fechar
</button>
</div>
</div>
...
{/* ESTILO NOBRE INSPIRADO 100% NO LAYOUT DO SEU PDF */}
<main className="max-w-4xl mx-auto px-4 md:px-8 mt-6">
  
  {/* Linha Divisória de Identidade com Fonte Serifada */}
  ...
  {categoriaSelecionada && (
    <>
      {/* Bloco de Horários dinâmicos e elegantes */}
      ...
      {/* Renderização das Subcategorias em GRID DE DUAS COLUNAS */}
      {categoriaSelecionada.subcategorias.map((subcategoria) => {
        ...
        return (
          <div key={subcategoria.nome} className="mb-10">
            ...
            {/* O GRID: Se adapta no Celular vs Computador */}
  ...
  {/* Rodapé Clássico das Folhas Impressas */}
...
diminuirQuantidade={disminuirQuantidade}
